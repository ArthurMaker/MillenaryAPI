...
import static Millenary.JSON.TextualComponent.rawText;
...
import Millenary.Utils.Reflection;
...
public class JSONMessage implements JsonRepresentedObject, Cloneable, Iterable<MessagePart>, ConfigurationSerializable {
	...
	public JSONMessage(final TextualComponent firstPartText) {
		...
		if(nmsPacketPlayOutChatConstructor == null){
			try {
				nmsPacketPlayOutChatConstructor = Reflection.getMinecraftClass("PacketPlayOutChat").getDeclaredConstructor(Reflection.getMinecraftClass("IChatBaseComponent"));
	...
	/**
	 * Set the behavior of the current editing component to instruct the client to open a file on the client side filesystem when the currently edited part of the {@code JSONMessage} is clicked.
	...
	/**
	 * Set the behavior of the current editing component to instruct the client to open a webpage in the client's web browser when the currently edited part of the {@code JSONMessage} is clicked.
	...
	/**
	 * Set the behavior of the current editing component to instruct the client to replace the chat input box content with the specified string when the currently edited part of the {@code JSONMessage} is clicked.
	...
	/**
	 * Set the behavior of the current editing component to instruct the client to send the specified string to the server as a chat message when the currently edited part of the {@code JSONMessage} is clicked.
	...
	public JSONMessage achievementTooltip(final Achievement which) {
		try {
			Object achievement = Reflection.getMethod(Reflection.getCraftBukkitClass("CraftStatistic"), "getNMSAchievement", Achievement.class).invoke(null, which);
			return achievementTooltip((String) Reflection.getField(Reflection.getMinecraftClass("Achievement"), "name").get(achievement));
		} catch (Exception e) {
			e.printStackTrace();
			return this;
		}
	...
	public JSONMessage statisticTooltip(final Statistic which) {
		...
		try {
			Object statistic = Reflection.getMethod(Reflection.getCraftBukkitClass("CraftStatistic"), "getNMSStatistic", Statistic.class).invoke(null, which);
			return achievementTooltip((String) Reflection.getField(Reflection.getMinecraftClass("Statistic"), "name").get(statistic));
		} catch (Exception e) {
			e.printStackTrace();
			return this;
		}
	...
	public JSONMessage statisticTooltip(final Statistic which, Material item) {
		...
		try {
			Object statistic = Reflection.getMethod(Reflection.getCraftBukkitClass("CraftStatistic"), "getMaterialStatistic", Statistic.class, Material.class).invoke(null, which, item);
			return achievementTooltip((String) Reflection.getField(Reflection.getMinecraftClass("Statistic"), "name").get(statistic));
		} catch (Exception e) {
			e.printStackTrace();
			return this;
	...
	public JSONMessage statisticTooltip(final Statistic which, EntityType entity) {
		...
		try {
			Object statistic = Reflection.getMethod(Reflection.getCraftBukkitClass("CraftStatistic"), "getEntityStatistic", Statistic.class, EntityType.class).invoke(null, which, entity);
			return achievementTooltip((String) Reflection.getField(Reflection.getMinecraftClass("Statistic"), "name").get(statistic));
		} catch (Exception e) {
			e.printStackTrace();
			return this;
	...
	public JSONMessage itemTooltip(final ItemStack itemStack) {
		try {
			Object nmsItem = Reflection.getMethod(Reflection.getCraftBukkitClass("inventory.CraftItemStack"), "asNMSCopy", ItemStack.class).invoke(null, itemStack);
			return itemTooltip(Reflection.getMethod(Reflection.getMinecraftClass("ItemStack"), "save", Reflection.getMinecraftClass("NBTTagCompound")).invoke(nmsItem, Reflection.getMinecraftClass("NBTTagCompound").newInstance()).toString());
		} catch (Exception e) {
			e.printStackTrace();
			return this;
		}
	...
	public void send(Player player){
		try {
			MillenaryAPI.getPacketFactory().sendPacket((Packet)createChatPacket(toJSONString()), player);
			//Object handle = Reflection.getHandle(player);
			//Object connection = Reflection.getField(handle.getClass(), "playerConnection").get(handle);
			//Reflection.getMethod(connection.getClass(), "sendPacket", Reflection.getMinecraftClass("Packet")).invoke(connection, createChatPacket(toJSONString()));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	private Object createChatPacket(String json) throws IllegalArgumentException, IllegalAccessException, InstantiationException, InvocationTargetException{
		if(nmsChatSerializerGsonInstance == null){
			...
			for(Field declaredField : Reflection.getMinecraftClass("ChatSerializer").getDeclaredFields()){
		...
		Object serializedChatComponent = nmsChatSerializerGsonInstance.fromJson(json, Reflection.getMinecraftClass("IChatBaseComponent"));
			...
			for(Map.Entry<String, JsonElement> entry : messagePart.entrySet()){
				...
				if(TextualComponent.isTextKey(entry.getKey())){
					...
					if(object.get("value").isJsonPrimitive()){
						...
						// The only composite type we currently store is another JSONMessage
