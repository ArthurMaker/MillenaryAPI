...
import Millenary.Factories.BlockFactory;
import Millenary.Factories.BossBarFactory;
import Millenary.Factories.DisguiseFactory;
import Millenary.Factories.HologramFactory;
import Millenary.Factories.InventoryFactory;
import Millenary.Factories.MobFactory;
import Millenary.Factories.DisguiseFactory.DisguiseType;
import Millenary.Factories.PacketFactory.PacketFactory;
import Millenary.Factories.Trader.TraderVillager;
import Millenary.Managers.BungeeCordManager;
import Millenary.MySQL.MakerSQL;
...
public class MillenaryAPI extends JavaPlugin {
	...
	public void onDisable(){
		packetfactory.getProtocolManager().disable();
		//mcdatabase.closeConnection();
	}
	
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args){
		if(cmd.getName().equalsIgnoreCase("api")){
			sender.sendMessage("§6MillenaryAPI");
			sender.sendMessage("§7 - Criado e desenvolvido por §larthurmaker.");
			sender.sendMessage("§7 2014 - §rhttp://www.millenarycraft.com");
			disguisefactory.disguiseAsMob(((Player)sender), DisguiseType.ENDER_DRAGON, true);
		}
		return false;
	}
	...
	private void registerOtherStuff(){
		registerEntity("CustomTrader", 120, TraderVillager.class);
		registerEntity("HNBat", 65, HNEntity.class);
	}
